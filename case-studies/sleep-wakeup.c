void waiter(struct wait_queue_head *wq_head, bool *condition)
{
	...
	for (;;) {
		...
		if (list_empty(&wait.entry))
			__add_wait_queue(wq_head, &wait); /* A */
...
void waker(struct wait_queue_head *wq_head, bool *condition)
{
	*condition = true; /* C */
	...
	/*
	 * Guarantee that we don't miss a wakeup.
	 *
	 * Orders C before D; matches the implicit smp_mb()
	 * in set_current_state() (A -> B).
	 *
	 * Forbids: (B ->fr C) and (D ->fr A)
	 */
	smp_mb();
	if (waitqueue_active(wq_head)) { /* D */
